        ...
        public static class ReducePierceDamageTaken
        {
            ...
            private static void Prefix(Character __instance, HitData hit)
            {
                if (__instance is Player player && hit.GetAttacker() is { } attacker && attacker != __instance)
                {
                    ...
                    {
                        if (player.GetEffectPower<Config>("FireResistance").Power > 0 || player.GetEffectPower<Config>("IntenseFire").Power > 0)
                        {
                            float dmg_reduce = ((100f - (player.GetEffectPower<Config>("FireResistance").Power + player.GetEffectPower<Config>("IntenseFire").Power)) / 100f);
